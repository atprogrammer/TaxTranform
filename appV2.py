...
# อัปโหลดไฟล์หลายไฟล์
#uploaded_files = st.file_uploader("เลือกไฟล์รูปภาพหรือ PDF", type=["png", "jpg", "jpeg", "pdf"], accept_multiple_files=True)
uploaded_files = st.file_uploader("เลือกไฟล์รูปภาพหรือ PDF", type=["pdf"], accept_multiple_files=True)
    ...
    for uploaded_file in uploaded_files:
        ...
        if file_type == "application/pdf":
            ...
            for i, image in enumerate(images):
                ...
                x1, y1, x2, y2 = 132, 670, 550, 750  # กำหนดพิกัดตามต้องการ
            ...
            st.image(image, caption=f"รูปภาพจากไฟล์ {uploaded_file.name}", use_container_width=True)

            # ตัวอย่างการ crop ภาพตามแกน x และ y: (x1, y1, x2, y2)
            x1, y1, x2, y2 = 100, 100, 500, 500  # กำหนดพิกัดตามต้องการ

            # Crop ภาพ
            cropped_image = image.crop((x1, y1, x2, y2))

            # แสดงผลภาพที่ crop ออกมา
            st.image(cropped_image, caption=f"ภาพที่ถูก crop จากไฟล์ {uploaded_file.name}", use_container_width=True)

            # ใช้ Tesseract OCR แปลงข้อความ
            text = pytesseract.image_to_string(cropped_image, lang="tha")  # เปลี่ยน "tha" เป็นภาษาอื่นหากต้องการ

            # ตัดคำนำหน้า ออกจากข้อความที่แปลงมา
            #text = text.replace("นาย", "").strip()
            #text = text.replace("นาง", "").strip()
            #text = text.replace("นางสาว", "").strip()

            all_text += f"{text}\n"

            # เปรียบเทียบข้อความและใช้หมายเลขบัตรประชาชนเป็นชื่อไฟล์
            new_id = find_id_from_name(text, df)
            if new_id:
                new_filename = f"{new_id}.pdf"  # ใช้เลขบัตรประชาชนเป็นชื่อไฟล์
            else:
                new_filename = f"{text[:50].replace(' ', '_').replace('\n', '')}.pdf"  # ใช้ข้อความเดิมถ้าไม่พบในฐานข้อมูล
            image.save(os.path.join(temp_dir, new_filename), "PDF")
